        ...
        private void FbDmTxbx_LostFocus(object sender, RoutedEventArgs e)
        {
            if (HousingDmTxbx.Text.Length > 0)
            {
                ...
                if (preCheck.CountRowInDB("potting", "housing_dm", HousingDmTxbx.Text) == 0)
                {
                    ...
                    {
                        ErrorLog.Create("potting", "housing_dm", HousingDmTxbx.Text,MethodBase.GetCurrentMethod().Name.ToString(), "Előző munkafolyamaton nem szerepelt a termék!", this.GetType().Name.ToString());
                    }
                }
