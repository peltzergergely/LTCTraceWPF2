        ...
        private void FbDmTxbx_LostFocus(object sender, RoutedEventArgs e)
        {
            if (HousingDmTxbx.Text.Length > 0)
            {
                ...
                if (preCheck.CountRowInDB("housing_leak_test_two", "housing_dm", HousingDmTxbx.Text) == 0)
                {
                    ...
                    {
                        ErrorLog.Create("housing_leak_test_two", "housing_dm", HousingDmTxbx.Text,MethodBase.GetCurrentMethod().Name.ToString(), "Előző munkafolyamaton nem szerepelt a termék!", this.GetType().Name.ToString());
                    }
                }
